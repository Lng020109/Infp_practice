...
public class MemberService {
	...
	// [수정] 8~16자, 특수문자 1개 이상 반드시 포함하도록 변경
	...
	private final java.util.List<String> forbiddenWords = java.util.Arrays.asList("바보", "qkqhdi", "멍청이", "audcjdgl",
			"시발", "씨발", "tlqkf", "씨바", "시바", "ㅅㅂ", "ㅆㅂ", "존나", "whssk", "병신", "qudtls", "개새끼", "쓰레기", "죽어", "꺼져");
	...
	public boolean register(MemberDTO dto) {
		...
		if (dto.getUsername() == null || dto.getPassword() == null || dto.getNickname() == null
				|| dto.getSecurityQuestion() == null || dto.getSecurityAnswer() == null) {
			System.out.println("❌ [회원가입 실패] 필수값 null 감지: " + "username=" + dto.getUsername() + ", password="
					+ dto.getPassword() + ", nickname=" + dto.getNickname() + ", question=" + dto.getSecurityQuestion()
					+ ", answer=" + dto.getSecurityAnswer());
			return false;
		}

		if (dto.getSecurityQuestion().trim().isEmpty() || dto.getSecurityAnswer().trim().isEmpty()) {
	...
	public String findUsername(String nickname, String question, String answer) {
		...
		Optional<Member> member = memberRepository.findByNicknameAndSecurityQuestionAndSecurityAnswer(nickname.trim(),
				question.trim(), answer.trim()); // [cite: 17]
	...
	public boolean resetPassword(String username, String question, String answer, String newPassword) {
		...
		Optional<Member> memberOpt = memberRepository
				.findByUsernameAndSecurityQuestionAndSecurityAnswer(username.trim(), question.trim(), answer.trim()); // [cite:
																														// 17]
	...
	// 프로필 이미지 저장
	// =========================
	public boolean updateProfileImage(String username, MultipartFile file) {
