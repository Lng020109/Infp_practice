...
function createPostHtml(post) {
    ...
    const userKey = post.username || post.author || "";
...
function openPost(id) {
    ...
    const defaultIcon = document.getElementById("detail-default-icon");

    if (userKey) {
        profileImg.src = `/api/member/${encodeURIComponent(userKey)}/profile-image`;
...
function closeDetailModal() {
    ...
    // 주소에서 게시글 id를 제거해서 새로고침해도 상세창이 다시 뜨지 않게 합니다.
    ...
    try {
        ...
        list.innerHTML =
            replies
                .map(
                    reply => {
                        // username이 있으면 우선 사용하고, 없으면 author 사용
                        const replyUserKey = reply.username || reply.author || "";
                        ...
                        return `
		                        ...
		                        <div class="reply-item">
		                            ...
		                            <div class="reply-user">
		                                ...
		                                <div class="reply-user-icon" style="width: 28px; height: 28px; border-radius: 50%; overflow: hidden; display: inline-flex; align-items: center; justify-content: center;">
                                ...
                                <b>
                                    ...
                                    ${escapeHtml(reply.author || reply.username)}
...
function checkPostOwner() {
    ...
    // 게시글 작성자의 username과 현재 로그인한 username 비교
    if ((currentPost.username || currentPost.author) === username) {
